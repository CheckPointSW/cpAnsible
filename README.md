...
6. Move the `cp_mgmt_api_python_sdk` directory from the `check_point_mgmt` directory, into `/usr/lib/python2.7/site-packages` or into  `/usr/lib/python2.7/dist-packages`, depending on the Python installation (from-source or none-source built Python, respectively). 
